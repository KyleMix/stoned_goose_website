...
- **Live OAuth login test.** The sandbox cannot reach stonedgoosecms.kylewmixon.workers.dev (network egress denied), so the sign-in loop needs one manual pass after merge: open /admin privately, sign in, save a trivial edit, confirm a content: commit lands and deploys.
- **ALLOWED_DOMAINS on the auth Worker.** Lives in the Cloudflare dashboard, not the repo. Confirm it lists exactly www.stonedgooseproductions.com (plus one explicit preview hostname if you edit from previews). SERVER_DEPLOYMENT.md now documents why wildcards are risky.
- **Upcoming-vs-past show filter in the editor.** Sveltia view_filters take static patterns only; date-relative filters are not supported in 0.170.0. Ticket-status filters plus default date sort stand in. Revisit on future Sveltia releases.
