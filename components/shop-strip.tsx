      ...
      <div className="mx-auto max-w-[1400px] px-5 md:px-10">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            >
              {shopCopy.heading}<span className="text-hazard">.</span>
            </h2>
          </div>
          <Link
            href="/shop"
        ...
        <ul className="group mt-12 grid grid-cols-1 gap-px overflow-hidden border border-bone/15 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((p) => (
            <ShopProductCard
              key={p.url}
              product={p}
