...
- Comics see **how many spots are left, never who has them**. The public endpoint returns counts and no endpoint returns a name without a token. That boundary lives in the Worker, not in the page: never add a client-side filter over a list of names and call it private.
- Two lifetimes, and the page states both. The **running order** is deleted when its Monday rotates off. The **comedian roster** is kept, for booking. Keeping it is only defensible because the sign up form says so and every confirmation email carries a removal link, so: do not remove the roster sentence from `signup.privacyNote`, do not disable the confirmation email while the roster is still being written, and do not make removal temporary. A removed row keeps `removed_at` set with the name blanked, so signing up again never silently re-adds someone.
- Three credentials, and they are not interchangeable. `OPEN_MIC_EXPORT_TOKEN` reaches contact details. `OPEN_MIC_HOST_TOKEN` reaches one night's running order with no emails, because it gets handed to whoever is working the room. Link tokens authorise one action on one row. Never widen the host route to include emails, and never serve a list off the export token where the host token would do.
- Email action links (`/open-mics/cancel`, `/open-mics/unsubscribe`) must stay **GET reads, POST writes**. Mail clients and security scanners fetch links nobody clicked, so a mutation on GET would cancel spots on its own.
